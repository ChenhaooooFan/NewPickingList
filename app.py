...
if st.button("🚀 点击生成分析报表") and this_week_file and last_week_file:
    ...
    sns.barplot(x=variation_counts.values, y=variation_counts.index, ax=ax)
    ax.set_xlabel('Count')
    ...
    sns.barplot(x=size_counts.values, y=size_counts.index, ax=ax)
    ...
    def allocate(size_row):
        ...
        if total_restock == 0:
            return pd.Series({'补S': 0, '补M': 0, '补L': 0, '⚠️库存预警': ''})
